...
<Link path="/guides/py-mcp-server">`py#mcp-server`</Link> または <Link path="/guides/ts-mcp-server">`ts#mcp-server`</Link> ジェネレーターを使用して作成した MCP サーバーを使用する場合は、<Link path="/guides/connection/ts-strands-agent-mcp">`connection` ジェネレーター</Link>を利用できます。
