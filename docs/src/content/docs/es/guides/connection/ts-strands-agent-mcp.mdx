...
El generador `connection` puede conectar tu <Link path="guides/ts-strands-agent">Agente Strands de TypeScript</Link> a un servidor MCP (ya sea <Link path="guides/ts-mcp-server">TypeScript</Link> o <Link path="guides/py-mcp-server">Python</Link>).
...
- packages/common/agent-connection
  - src
    ...
    - core
      - agentcore-mcp-client.ts Cliente MCP de AgentCore de bajo nivel con autenticación SigV4/JWT
...
- Actualiza el objetivo `serve-local` del agente para depender del objetivo serve del servidor MCP
...
El parámetro `sessionId` se pasa desde el llamador, asegurando consistencia para la [Observabilidad de Bedrock AgentCore](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/observability.html).
...
El ARN del runtime AgentCore del servidor MCP se registra automáticamente en el espacio de nombres `connection` de la <Link path="guides/runtime-config">Configuración de Runtime</Link> por el constructo CDK generado, para que el agente pueda descubrirlo en tiempo de ejecución.
</Fragment>
...
El ARN del runtime AgentCore del servidor MCP se registra automáticamente en el espacio de nombres `connection` de la <Link path="guides/runtime-config">Configuración de Runtime</Link> por el módulo Terraform generado, para que el agente pueda descubrirlo en tiempo de ejecución.
</Fragment>
</Infrastructure>
