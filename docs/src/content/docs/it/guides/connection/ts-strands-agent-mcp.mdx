...
Il generatore configura tutto il cablaggio necessario affinché il tuo agente possa scoprire e invocare gli strumenti del server MCP, sia quando distribuito su AWS (tramite Bedrock AgentCore) che durante l'esecuzione locale.
...
<Infrastructure>
<Fragment slot="cdk">
Dopo aver eseguito il generatore di connessione, è necessario concedere all'agente il permesso di invocare il server MCP:
...
Dopo aver eseguito il generatore di connessione, è necessario concedere all'agente il permesso di invocare il server MCP nella tua configurazione Terraform:
...
Questo avvierà sia l'agente che tutti i server MCP connessi, con l'agente che si connette ai server MCP direttamente tramite HTTP sulle porte locali assegnate.