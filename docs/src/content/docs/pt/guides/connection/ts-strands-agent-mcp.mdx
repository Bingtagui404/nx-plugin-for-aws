...
- Atualiza o destino `serve-local` do agente para depender do destino serve do servidor MCP
...
O ARN do runtime AgentCore do servidor MCP é automaticamente registrado no namespace `connection` da <Link path="guides/runtime-config">Runtime Configuration</Link> pelo construto CDK gerado, para que o agente possa descobri-lo em tempo de execução.
</Fragment>
...
O ARN do runtime AgentCore do servidor MCP é automaticamente registrado no namespace `connection` da <Link path="guides/runtime-config">Runtime Configuration</Link> pelo módulo Terraform gerado, para que o agente possa descobri-lo em tempo de execução.
</Fragment>
</Infrastructure>

## Desenvolvimento Local

O gerador configura o destino `serve-local` do agente para:
...
Isso iniciará tanto o agente quanto todos os servidores MCP conectados, com o agente conectando-se aos servidores MCP diretamente via HTTP nas suas portas locais atribuídas.